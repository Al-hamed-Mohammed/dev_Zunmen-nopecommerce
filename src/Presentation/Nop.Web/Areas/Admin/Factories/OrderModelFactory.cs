...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        protected virtual void PrepareReturnRequestBriefModels(IList<OrderItemModel.ReturnRequestBriefModel> models, OrderItem orderItem)
        {
            ...
            var returnRequests= _returnRequestService.SearchReturnRequests(vendorID: 0, orderItemId: orderItem.Id);

            if (_workContext.IsAdmin)
            {
                 returnRequests = _returnRequestService.SearchReturnRequests(vendorID: 0, orderItemId: orderItem.Id);
            }
            else
            {
                 returnRequests = _returnRequestService.SearchReturnRequests(vendorID: _workContext.CurrentVendor.Id, orderItemId: orderItem.Id);
            }
