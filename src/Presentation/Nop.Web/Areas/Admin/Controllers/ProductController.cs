...
namespace Nop.Web.Areas.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        public ProductController(IAclService aclService,
            ...
            IWalletDetailService walletDetailService
            )
        {
        ...
        public virtual IActionResult Create()
        {
            ...
            var wallet = _walletService.GetAllWallet()
                 .Where(w => w.CustomerID == _workContext.CurrentCustomer.Id)
                 .FirstOrDefault();
            ...
            if (wallet != null)
            {
                if (wallet.Amount <= 0)
                {
                    _notificationService.ErrorNotification(_localizationService.GetResource("Admin.Catalog.Products.InsufficientAmount"));
                    return RedirectToAction("List");
                }
            }
            else
        ...
        public virtual IActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //Removing Wallet Amount
                var wallet = _walletService.GetAllWallet()
                 .Where(w => w.CustomerID == _workContext.CurrentCustomer.Id)
                 .FirstOrDefault();

                decimal walletRedecedAmountAfterChargesApply = wallet.Amount - WalletProduct.ProductCharge;

                wallet.Amount = walletRedecedAmountAfterChargesApply;

                _walletService.UpdateWallet(wallet);

                // Add Record to Wallet Details Table
                var walletdetail = new WalletDetail
                {
                    WalletID = wallet.Id,
                    Description = "ProductID- " + product.Id + " Details- " + product.Name + " product added",
                    DateCreated = DateTime.Now,
                    Amount = WalletProduct.ProductCharge,
                    AmountAdded = false,
                    AmountDeducted = true,
                    IsActive = true
                };

                _walletDetailService.InsertWalletDetail(walletdetail);
